...
export default function Operation() {
  ...
  // Create an object to store added operations based on their listId
  const [addedOperationsByListId, setAddedOperationsByListId] = useState({});
  ...
  useEffect(() => {
    ...
    // Fetch added operations based on the current listId
    fetchAddedOperationsByListId(listId);

    fetchOperationLibs();
    fetchOperationList();
  }, [listId, jobId, bundleId]); // re-fetch when these parameters change

  // Function to fetch added operations based on listId
  const fetchAddedOperationsByListId = (listId) => {
    // Retrieve added operations from the object based on the current listId
    const addedOperations = addedOperationsByListId[listId] || [];
    setOperationList(addedOperations);
  };
  ...
  const handleAddOperation = async (selectedOperation) => {
    try {
      ...
      // Update the addedOperations object to include the newly added operation
      const updatedAddedOperations = { ...addedOperationsByListId };
      if (!updatedAddedOperations[listId]) {
        updatedAddedOperations[listId] = [];
      }
      updatedAddedOperations[listId].push(newOperation);
      setAddedOperationsByListId(updatedAddedOperations);
    } catch (error) {
      console.error("Error adding operation:", error);
    }
  };
  ...
  const handleDelete = async (operationId) => {
    try {
      ...
      // Remove the deleted operation from the addedOperations object based on listId
      const updatedAddedOperations = { ...addedOperationsByListId };
      if (updatedAddedOperations[listId]) {
        updatedAddedOperations[listId] = updatedAddedOperations[listId].filter(op => op.id !== operationId);
        setAddedOperationsByListId(updatedAddedOperations);
      }
    } catch (error) {
      console.error("Error deleting operation:", error);
