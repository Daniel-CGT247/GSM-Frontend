...
//     axios.delete(`http://127.0.0.1:8000/element_list/${uniqueId}`, {
//         headers: {
//           Authorization: `JWT ${localStorage.getItem("access_token")}`,
//         },
//       })
...
const ElementLibList = () => {
  ...
  const [currentOperation, setCurrentOperation] = useState(null);
  useEffect(() => {
    axios.get(`${endpoint}/operation_lib/${operationId}`, {
      headers: {
        Authorization: `JWT ${localStorage.getItem("access_token")}`,
      },
    })
  ...
  useEffect(() => {
    axios.get(`${endpoint}/element_list/`, {
        params: { operationList: operationListId },
        headers: {
          Authorization: `JWT ${localStorage.getItem("access_token")}`,
        },
  ...
  const handleDeleteElement = (uniqueId) => {
    axios.delete(`${endpoint}/element_list/${uniqueId}`, {
        headers: {
          Authorization: `JWT ${localStorage.getItem("access_token")}`,
        },
      })
    ...
    <Container fluid="lg" className="pt-5">
      <Row>
        ...
        <Row className="mt-4 d-flex justify-content-center">
          <Col xs="auto">
            <Button variant="success" onClick={() => navigate(-1)}>Complete</Button>
          </Col>
        </Row>
      ...
      <Row>
        <InputGroup className="mb-6">
          <FormControl placeholder="Search Elements..." value={searchFilter} onChange={handleSearchChange} />
        </InputGroup>
        
        <Col md={6}>
          <Card>
            <Card.Header>Element Library</Card.Header>
            <Card.Body style={{ maxHeight: '550px', overflowY: 'auto', minHeight: '550px' }}>
              <Table>
                <thead>
                  <tr>
                    <th style={{ width: '5%' }}>#</th>
                ...
                <tbody>
                  {currentItems.map((elementLib, index) => (
                    <tr key={elementLib.id}>
                      ...
                      <td>
                        {elementLib.variables.length > 0 ? (
                          <Button variant="link" onClick={() => toggleOptionsVisibility(elementLib.id)} style={{ textDecoration: 'none' }}>
                            {visibleOptions[elementLib.id] ? <ChevronUp /> : <ChevronDown />}
                            {" Available"}
                          ...
                          <div>
                            {elementLib.variables.map((variable, index) => (
                              <div key={index} className="mb-2">
                                <label htmlFor={variable.name} className="form-label d-block">
                                  {variable.name}:
                                </label>
                                <select
                                  className="form-select"
            ...
            <Card.Footer>
              <Pagination className="justify-content-center">{renderPaginationItems(filteredElements.length, itemsPerPage)}</Pagination>
            </Card.Footer>
          </Card>
        </Col>
