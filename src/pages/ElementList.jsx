...
// //           )
// //         : elementLibList;
// //       setFilteredLibs(filtered); // Update this state to reflect the current filtered items
// //     };

// //     // Call this function whenever the searchFilter or elementLibList changes
// //     handleFilterElements();
// //   }, [searchFilter, elementLibList]);

// //   const handleSearchChange = (event) => {
// //     setSearchFilter(event.target.value);
// //   };

// //   const pageCount = Math.ceil(filteredLibs.length / itemsPerPage);
...
// //                     </Td> */}
// // <Td>
// //   {element.selectedOptions?.map((option, index) => (
// //     <Box key={index}>{option}</Box> // Displaying each selected option
// //   ))}
// // </Td>
...
import { Search2Icon, CloseIcon, ChevronLeftIcon, ChevronRightIcon } from "@chakra-ui/icons";
import TableSkeleton from "../components/TableSkeleton";
...
export default function ElementList() {
  ...
  const handleAddNewExpandingName = async (uniqueId) => {
    if (newExpandingName.trim() !== "") {
      setNewExpandingName("");
      const selectedElement = selectedElements.find((element) => element.uniqueId === uniqueId);
      if (!selectedElement) {
        console.error(`Element with uniqueId ${uniqueId} not found.`);
        return;
      }
  ...
  useEffect(() => {
    const fetchExpandingNames = async () => {
      try {
        const elementListResponse = await axios.get(`${endpoint}/element_list/`, {
          params: { listItem_id: operationListId },
          headers: headers,
        });
        const uniqueExpandingNames = [
          ...new Set(elementListResponse.data.map((item) => item.expanding_name)),
        ];
        setExpandingNamesList(uniqueExpandingNames.filter((name) => name !== null));
      } catch (error) {
        console.error("Error fetching expanding names:", error);
      }
    };
  ...
  const indexOfLastItem = currentPage * itemsPerPage;
  const indexOfFirstItem = indexOfLastItem - itemsPerPage;
  const currentItems = selectedElements.slice(indexOfFirstItem, indexOfLastItem);

  return (
    <Card>
      <CardBody>
        {loading ? (
          <TableSkeleton header="Element List" columns={["#", "Name", "Expanding Field", "Time", "Selected Options", "Action"]} />
        ) : (
          <Flex direction="column" maxW="4xl" margin="auto">
            <Box height="650px" overflowY="auto" maxH="75vh" mb="4">
              <Flex justifyContent="space-between" alignItems="center" mb="4">
